 ...
 *  100 feet per minute, one zone long, releasing a zone at the start of leaving it. The stations sit
 *  on the intersections Arena's own station table names -- entry at I6, workstation at I1, paint at
 *  I2, new paint at I4, packing at I3, exit at I5 -- and the staging area is I12.
 *
 *  ## What agreement means here
 *
 *  Both models are stochastic, so z is the gap between the means over the two half-widths combined
 *  in quadrature, and **z <= 1 is agreement at 95%** for two independent estimates. Ten replications
 *  of 600,000 minutes with a 50,000 minute warm-up, on both sides.
 *
 ...
 *
 *  Arena's model keeps its own counter, `vNumWaitingRequests`, and reports its mean as 0.677.
 *  Requests arrive at 78,463 per 550,000 minutes, so whatever that counter counts, it counts each
 *  request for 4.75 minutes on average. The queue wait alone is 0.53. The counter therefore runs
 *  until the vehicle arrives, not until it is assigned, and the wider reading is the one Arena ran.
 *
 *  Both are run below. The wider reading is the primary case; the narrower is run beside it, and
 *  it is worse on the shop queues and worse on the fleet.
 *
 *  ## Where the two tools genuinely differ
 *
 *  Arena has no way to move an unallocated transporter under this model's "When Freed: Remain"
 *  setting, so the staging trip is made as an ordinary **transport**: the entity that has just been
 *  set down is duplicated, and the copy keeps the vehicle and rides it to I12 before freeing it.
 *  That is not read off the numbers -- the module list has the Separate, the Transport to the
 *  staging station, and the Free that follows it. A vehicle on its way to the staging area is
 *  therefore *busy* and *cannot be diverted*.
 *
 ...
 *
 *  Committed vehicle time -- allocated plus repositioning, which is the one figure Arena reports as
 *  busy and which this subsystem splits in two:
 *
 *  ```
 *                             allocated   repositioning   committed
 *  Arena (busy, one figure)           --              --     0.6930
 *  KSL, until collected           0.5768          0.1318     0.7087
 *  KSL, queued only               0.5405          0.2567     0.7972
 *  ```
 *
 *  Under the reading Arena ran, the two fleets are committed for the same share of the horizon to
 *  within 2.3%, and the whole of the 69.76 that `agvUtilization` scores is the classification rather
 *  than the quantity. That is asserted below, not merely printed.
 *
 ...
 *  of loading and unloading, the waits for a vehicle, and the nominal loaded travel of 8.1 minutes a
 *  part for the old route and 8.3 for the new -- leaves the time a part spends being fetched and
 *  being held up:
 *
 *  ```
 ...
 *  against 4.62 in Arena. (Arena reports no such figure; 4.62 is what its own numbers leave over,
 *  by the same subtraction. The residual is empty running *plus* blocking while loaded, since a
 *  journey's elapsed time contains its own blocking -- an earlier reading of this that took it for
 *  empty running alone was wrong by the two minutes a part that blocking accounts for.)
 *
 ...
 *  vehicle that cannot move until whatever is in I12 is dispatched. Arena never offers that vehicle
 *  at all, and the obstruction count below says the situation arises about 25,000 times a
 *  replication.
 *
 *  The paint queue is the one station statistic that does not agree, at 3.13 minutes against 2.51.
 *  It is the most sensitive of the four and the least about the paint station: the workstation ahead
 *  of it runs at 82% on a nearly deterministic 21-to-25-minute service, so what arrives at the paint
 *  station is a smoothed stream whose remaining variability is almost entirely put there by the
 *  transport. A queue that would be 13.8 minutes under Poisson arrivals is 2.5, and it moves with
 *  the aisle rather than with the shop.
 *
 ...
 *  is what the exercise says should happen -- "if there are multiple vehicles idle, they should wait
 *  on Link 11". The diagnostic's own advice, to give idle transporters a staging area, is what this
 *  model already does. The count is reported below rather than suppressed, because it is a true
 *  reading of a real cost: those vehicles are idle, in the way, and cannot leave.
 */
class PaintingFlowLineArenaCrossCheckTest {
     ...
     *  the staging area, and prints both against the single figure Arena reports.
     *
     ...
     *  vehicle spends travelling *on a job* -- empty out and loaded back, blocking included, because
     *  a journey's elapsed time contains its own blocking -- is what each part reports as its
     *  transfer time less the four minutes per leg of loading and unloading. The remainder is
     *  repositioning, which Arena counts as busy and this subsystem does not.
     */
    private fun reportCommitment(shop: PaintingFlowLineWithAgvs, arenaBusy: Double) {
        ...
        println("    allocated %.4f + repositioning %.4f = committed %.4f   (Arena reports busy %.4f)"
            .format(allocated, repositioning, allocated + repositioning, arenaBusy))
        println("    empty move per leg %.4f minutes; obstructions detected per replication %.0f"
            .format(
                shop.emptyMoveTime.acrossReplicationStatistic.average,
    ...
    fun threeAgvsOnAOneWayNetwork() = withoutObstructionWarnings {
        val arena = arenaFixture("/arena/P7-15.csv")
        val arenaBusy = arena.getValue("agvUtilization").first

        val shop = buildAndRun(PendingRequestTest.UNTIL_COLLECTED)
        val measured = measure(shop)
        val z = compare(
            "Exercise 7.15: three AGVs, one-way guide path, staging area at I12, KSL against Arena\n" +
                    "  a request counts as pending until a vehicle reaches the part, which is what " +
                    "Arena's own counter measures",
            arena, measured
        )
        reportCommitment(shop, arenaBusy)
        ...
        val zQueuedOnly = compare(
            "  ...and counting only parts actually queued, for comparison", arena, measure(queuedOnly)
        )
        reportCommitment(queuedOnly, arenaBusy)
        ...
        // The wider reading is Arena's. It is better on the shop queues, which is where the choice
        ...
        for (name in listOf("workStationQueueWait", "packQueueWait")) {
            assertTrue(
                ...
                "counting a request pending until collection should fit Arena better on $name, " +
        ...
        // classified. Arena counts the trip to the staging area as busy; this subsystem does not.
        val vehicles = shop.transportSystem.transporters
        val travelAndBlock = vehicles.sumOf {
            it.fracTimeMoving.acrossReplicationStatistic.average +
                    it.fracTimeBlocked.acrossReplicationStatistic.average
        ...
        assertTrue(
            abs(committed - arenaBusy) / arenaBusy <= 0.05,
            "committed vehicle time should account for Arena's busy figure to within 5%%, but was " +
                    "%.4f against %.4f".format(committed, arenaBusy)
        )
    }
}
